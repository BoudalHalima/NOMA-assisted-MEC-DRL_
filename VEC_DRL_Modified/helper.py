...
from ddpg_lib import *   #函数使用不用标注库的来源
import ipdb as pdb
...
def complexGaussian(row=1, col=1, amp=1.0):  #产生复合高斯噪声
    real = np.random.normal(size=[row,col])[0]*np.sqrt(0.5)   #正态分布随机取值
    img = np.random.normal(size=[row,col])[0]*np.sqrt(0.5)
    return amp*(real + 1j*img)

class ARModel(object):   #自回归模型
    """docstring for AR channel Model"""
    def __init__(self, n_t=1, n_r=1, seed=123):
        self.n_t = n_t
        self.n_r = n_r
        np.random.seed([seed])       
        self.H = complexGaussian(self.n_t, self.n_r)     #高斯函数  hsm(t) 初始化
        
    def getCh(self,dis,lane):   #返回最原始的hm(t)  (5)
        dis_lane = width_lane*lane
        self.dis = dis
        self.position = np.array([self.dis,dis_lane,Hight_RSU])
        self.path_loss = ref_loss/np.power(np.linalg.norm(self.position),2)  #路径损失对应hr，ref_loss为固定值
        return self.H*np.sqrt(self.path_loss)  #(5)
        
    def sampleCh(self,dis,rho,lane): #求取下一时刻的hm(t)
        self.H = rho*self.H + complexGaussian(self.n_t, self.n_r, np.sqrt(1-rho*rho))  #hsm (7)
...
class DDPGAgent(object):  #DDPG大汇总   大部分函数来自于ddpg.lib文件
    ...
    def __init__(self, sess, user_config, train_config):
        ...
        self.replay_buffer = ReplayBuffer(int(train_config['buffer_size']), int(train_config['random_seed']))  #Bm（t）的更新
    ...
    def predict(self, s, isUpdateActor):  #加噪声探索
        if isUpdateActor:
    ...
    def update(self, s, a, r, t, s2, isUpdateActor):    #储蓄池
