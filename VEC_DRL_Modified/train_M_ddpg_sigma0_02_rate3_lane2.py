...
for k in range(1,10):
    # New computation graph is created for each iteration, preventing interference between different graphs
    ...
    user_list = [];  # store instances of the MecTermRL class for each user
    
    for info in user_config:
        info.update(config) # updates the dictionary info by adding key-value pairs from the config dictionary
    ...
    res_r = []
    res_p = []
    res_p_offload = []  # To store the total powers used for offloading by each user.
    res_p_local=[]      # To store the total powers used for local processing by each user.
    res_b = []          #  To store the buffer lengths of each user.
    res_o = []          # To store the overflow probabilities of each user.
    res_d = []          # To store the number of successfully offloaded tasks of each user.
    ...
    for i in range(MAX_EPISODE):
        ...
        for j in range(MAX_EPISODE_LEN):
            ...
            cur_r_ep += cur_r       # Total reward
            cur_p_ep += cur_p       # Total power used
            cur_op_ep += cur_op     # Total offloading power used
            cur_ts_ep += cur_ts     # Total transmitted bits
            cur_ps_ep += cur_ps     # Total processed bits locally
            cur_rs_ep += cur_rs     # Total received bits
            cur_ds_ep += cur_ds     # Total data in the buffer
            cur_ch_ep += cur_ch     # Total channel gain
            cur_of_ep += cur_of     # Total number of successfully offloaded tasks
            # print (temp)
            cur_noise_ep += temp
            cur_power_local += cur_pl       # Total power used for local processing
            cur_power_offload += cur_pt     # Total power used for offloading
            ...
            if done:
                ...
                try:
                    '''removes the previous lines from the plots to update them with the latest data. 
                    It ensures that the plots only show the most recent data points and avoid clutter'''
